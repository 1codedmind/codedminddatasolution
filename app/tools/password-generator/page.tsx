  ...
  return (
    <main className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8">
        ...
        <p className="text-stone-500 mt-1">Generate strong, random passwords in your browser.</p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {(Object.keys(CHARS) as Array<keyof typeof CHARS>).map((key) => (
            <label key={key} className="flex items-center gap-3 cursor-pointer select-none">
              ...
              />
              <span className="text-sm text-stone-700 capitalize">{key === "upper" ? "Uppercase (A–Z)" : key === "lower" ? "Lowercase (a–z)" : key === "numbers" ? "Numbers (0–9)" : "Symbols (!@#…)"}</span>
            </label>
          ))}
        </div>
